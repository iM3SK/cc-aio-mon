...
- **shared.py** — shared helpers (`_num`, `_sanitize`, `f_dur`, `f_tok`, `f_cost`, `calc_rates`, `char_width`, `is_safe_dir`, `ensure_data_dir`, `strip_context_suffix`, `compact_context_suffix`, `extract_changelog_entry`, `run_git`), constants (`RESERVED_SIDS`), ANSI color constants, and regexes (`_SID_RE`, `_ANSI_RE`) used by statusline.py, monitor.py, pulse.py, and update.py
